...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  HomeBloc(this.myGaragesUseCase, this.myOrdersUseCase, {int initialSelectedStatus = 1})
    ...
    on<GetMyGaragesEvent>((event, emit) async {
      ...
      result.fold(
            (error) => emit(state.copyWith(myGaragesState: RequestState.error, myGaragesErrorMessage: error.message)),
            (data) => emit(state.copyWith(myGaragesState: RequestState.loaded, data: data)),
      );
    });

    on<GetMyOrdersEvent>((event, emit) async {
      emit(state.copyWith(selectedStatus: event.newStatus, myOrdersState: RequestState.loading));
      final result = await myOrdersUseCase.myOrders(event.newStatus);
      result.fold(
            (error) => emit(state.copyWith(myOrdersState: RequestState.error, myOrdersErrorMessage: error.message)),
